...
- **Models** (`datasetapp/models.py`): `Tag`, `Dataset`, `DataFile`, `Hit`.
  ...
  - `Dataset` ↔ `DataFile` is one-to-many (a dataset can have CSV + XLSX + XML + MAT siblings).
  - `Hit` records one download. Since v1.3.0 (#17) the table holds only `(dataset_hit, date_and_time)` — no IP, User-Agent, or referrer is captured. Two columns power three visible features: the per-dataset count on the detail page, the sortable Downloads column on the homepage / tag-filter list (v1.7.0, via a `Count("datafile__hit", distinct=True)` annotation in `_annotate_with_downloads`; desktop-only — the mobile card layout hides it), and the seven-year sparkline on the detail page (`_download_series` in `datasetapp/views.py` groups rows by `TruncWeek("date_and_time")` and reports the total weekly download count — v1.9.1, issue #104). Rows are kept indefinitely; the per-row history is fine to retain because no PII is stored.
...
- **Templates** (`datasetapp/templates/datasetapp/`):
  ...
  - `dataset_info.html` — the detail page. Pulls in MathJax for any LaTeX in descriptions, renders a seven-year weekly-average downloads sparkline (ECharts) below the download counter, a Python quickstart code block with a copy-to-clipboard button (rendered when a CSV file exists), an inline CSV preview table (first 10 rows) above the download block, a Share button next to the title that invokes the Web Share API and falls back to clipboard-copy when `navigator.share` is missing (v1.11.0), and a top-of-page topbar with three chip buttons — Previous / Back to home / Next — on a 1fr-auto-1fr grid so the centre "home" chip stays anchored when either prev or next is missing (v1.12.0), plus a bottom-of-page repeat of just the Previous / Next chips (`.detail-topbar--bottom`, same `prev_dataset` / `next_dataset` slug-ordering context, no "home" chip) so navigation is reachable after scrolling (v1.15.0).
...
5. **`download_dataset` validates its `file_name` against `^[a-z0-9-]+\.[a-z]{3,4}$` before any DB lookup.** Anything outside that shape returns 404, not 500. The regex accepts a 3- or 4-letter extension (the `XLSX` file type, issue #113); the `_DOWNLOAD_EXT_ALIASES` map then folds the legacy `.xls` extension onto the now-`XLSX` row so pre-#113 `/file/<slug>.xls` links still resolve. If you ever need to support a new extension (`.json`, `.tsv`, …), update `_DOWNLOAD_FILENAME_RE` *and* the `DataFile.file_type_choice` tuple in the same change.
