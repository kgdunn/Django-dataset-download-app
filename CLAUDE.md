...
- **Caddy config**: `/etc/caddy/Caddyfile` on the host — canonical structure documented in [`docs/caddy.md`](docs/caddy.md). Reload with `sudo systemctl reload caddy` (validate first with `sudo caddy validate --config /etc/caddy/Caddyfile`).
- **TLS**: prod and staging hostnames must live in **separate Caddy site blocks** because they need different certs. Sharing a single block re-broke `test.openmv.net` for direct (non-Cloudflare) clients in issue #89.
  - `openmv.net` and `www.openmv.net` use a **Cloudflare Origin Certificate** (15-year, signed by Cloudflare's internal CA) at `/etc/caddy/origin-certs/openmv.net/`. Cloudflare's edge serves a public-trusted cert to visitors and re-encrypts to origin in `Full (strict)` mode. The Origin CA is **not** in any browser or Python trust store, so this cert is only valid when consumed by Cloudflare's edge.
  - `test.openmv.net` uses Caddy-managed Let's Encrypt — its site block has **no `tls` directive**, so Caddy's automatic-HTTPS path issues a public-trust cert via ACME HTTP-01. It must stay **DNS-only** (grey cloud) in Cloudflare so the challenge can reach origin directly. If you ever see `ERR_CERT_AUTHORITY_INVALID` / `SSL: CERTIFICATE_VERIFY_FAILED` on `test.openmv.net`, the site block has merged with the prod one and is serving the Origin CA cert — see `docs/caddy.md`.
