...
def test_download_series_uses_cache(client, db):
    ...
    assert first == second
    # After clearing the cache the new Hit should appear. The series now
    # reports total downloads per week, so two hits in the current week
    # sum to 2 (issue #104 follow-up).
    cache.clear()
