...
| 11 | Low     | `.github/workflows/ci.yml`                               | No `permissions:` block; `actions/checkout@v4` and `astral-sh/setup-uv@v6` referenced by floating tag. (`deploy.yml` is already fine.)                                                           | **Fixed in v1.8.1** — `permissions: contents: read` and `pip-audit` step added in v1.5.0; in v1.8.1 both `actions/checkout` and `astral-sh/setup-uv` pinned by full 40-char commit SHA in `ci.yml` and `release.yml`, with `# vX.Y.Z` comments, and `.github/dependabot.yml` added (`github-actions` + `docker` ecosystems, weekly) to keep the pins fresh. Closes Issue J. |
...
- **Status:** **Fixed in v1.8.1** — `python:3.11-slim` pinned by `@sha256:` digest in both stages of `Dockerfile`; `postgres:16-alpine` pinned by `@sha256:` digest in `docker-compose.prod.yml` *and* in the CI workflow's Postgres service container so the two stay byte-identical; `.github/dependabot.yml` adds the `docker` ecosystem on a weekly schedule. Closes Issue H.
...
- **Status:** **Fixed in v1.8.1** — `actions/checkout` pinned to v4.3.1 (`34e114876b0b11c390a56381ad16ebd13914f8d5`) in both `ci.yml` and `release.yml`; `astral-sh/setup-uv` pinned to v6.8.0 (`d0cc045d04ccac9d8b7881df0226f9e82c39688e`) in `ci.yml`; each pin carries a `# vX.Y.Z` comment so a human can audit at a glance. `.github/dependabot.yml` adds the `github-actions` ecosystem on a weekly schedule (alongside `docker` for Issue H). Closes Issue J. The companion finding 11 in the audit table is now closed too.
